...
#' res = d_selection_G(X, Y, S = c(1:7), g.hat = g2, monotone=TRUE, B=100)
#' d_selection_G
#'
#' @description  It performs closed testing method with Shiraishi local test
#' using an exact shortcut valid when the outlier density is monotone, either
...
#' @param g.hat  it can be either a character ("analytical") or a function denoting the outlier density.
#' If \code{NULL}, the outlier density is estimated from the data.
#' @param monotone Boolean. \code{TRUE}: assume monotone, use the monotone-shortcut.
#' \code{FALSE} or \code{NULL}: do not assume monotone, use the conservative shortcut.
#' @param fit_method  "betamix" or "mixmodel".
#' @param prop_cal   proportion of inliers used to estimate the inlier distribution.
#' @param alpha  significance level.
#' @param pvalue_only  logical. If \code{TRUE}, only the global test is performed.
#' @param n_perm  minimum test sample size needed to use the asymptotic distribution.
#' @param B  replications for critical values and global *p*-value.
#' @param B_MC  replications for the Shiraishi test statistic.
#' @param seed  RNG seed.
#' @param use_b_approx  logical. If \code{TRUE}, replace Monte Carlo estimation
#' of the score vector \eqn{a^l_r = E[g(U^{(r)}_{m+l})]} with the deterministic
#' plug-in approximation \eqn{b^l_r = g(r/(m+l+1))}.
#'
#' @export
d_selection_G <- function(S_X, S_Y, S=NULL, k=NULL, g.hat=NULL, monotone=NULL,
                          fit_method="betamix", prop_cal=0.5, alpha=0.1,
                          pvalue_only=FALSE, n_perm=10, B=10^3, B_MC=10^3,
                          seed=123, use_b_approx=FALSE) {

    if (!is.null(monotone)) {
        monotone <- toupper(monotone)
        if (monotone == "F" || monotone == F) monotone <- FALSE
        if (monotone == "T" || monotone == T) monotone <- TRUE
        stopifnot(
            "Error: monotone must be logical value." = is.logical(monotone),
            length(monotone) == 1
        )
    }
    ...
    if (is.null(g.hat)) {
        ## Treat both monotone=FALSE and monotone=NULL as "do not assume monotone".
        ## Only monotone=TRUE turns on the monotone-density shortcut.
        use_monotone <- isTRUE(monotone)
        monotone2 <- use_monotone
        m1 <- round(prop_cal * m)
        ...
        S_pooled <- c(S_X2, S_Y)
        g.fit <- estimate_g(S_X1, S_pooled, method = fit_method,
                            monotone = monotone2)
        g.hat <- g.fit$pdf
        ## Determine direction so d_G_monotone runs the right worst-case
        ## argument. If estimate_g picked "decreasing", we must run with decr=TRUE.
        g.decr <- isTRUE(!is.null(g.fit$monotonicity) && g.fit$monotonicity == "decreasing")

        if (!use_monotone) {
            res <- d_G_cons(S_X=S_X2, S_Y=S_Y, S=S, g.hat=g.hat, k=k,
                            alpha=alpha, pvalue_only=pvalue_only,
                            n_perm=n_perm, B=B, B_MC=B_MC, seed=seed,
                            use_b_approx=use_b_approx)
        } else {
            res <- d_G_monotone(S_X=S_X2, S_Y=S_Y, S=S, g.hat=g.hat,
                                decr=g.decr, k=k, alpha=alpha,
                                pvalue_only=pvalue_only, n_perm=n_perm,
                                B=B, B_MC=B_MC, seed=seed,
                                use_b_approx=use_b_approx)
        }
    } else {
        ## Same dispatch for user-supplied g.hat
        use_monotone <- isTRUE(monotone)
        if (!use_monotone) {
            res <- d_G_cons(S_X=S_X, S_Y=S_Y, S=S, g.hat=g.hat, k=k,
                            alpha=alpha, pvalue_only=pvalue_only,
                            n_perm=n_perm, B=B, B_MC=B_MC, seed=seed,
                            use_b_approx=use_b_approx)
        } else {
            ## Detect direction of user-supplied g.hat
            u <- seq(0.001, 0.999, length.out = 200)
            gu <- g.hat(u)
            if (all(diff(gu) >= -1e-8)) {
                g.decr <- FALSE
            } else if (all(diff(gu) <= 1e-8)) {
                g.decr <- TRUE
            } else {
                stop("monotone=TRUE but supplied g.hat is not monotone on (0,1).")
            }
            ## Default for d_G_monotone is monotonicity increasing.
            res <- d_G_monotone(S_X=S_X, S_Y=S_Y, S=S, g.hat=g.hat, decr=g.decr, k=k,
                                alpha=alpha, pvalue_only=pvalue_only,
                                n_perm=n_perm, B=B, B_MC=B_MC, seed=seed,
                                use_b_approx=use_b_approx)
        }
...
#' d_G_monotone
#'
#' Monotone-shortcut Shiraishi closed test.
#'
#' @param decr logical. If TRUE, use the decreasing-g branch.
#' @param use_b_approx logical. See d_selection_G.
#'
#' @export
d_G_monotone <- function(S_X, S_Y, S=NULL, g.hat, decr=FALSE, k=NULL,
                         alpha=0.1, pvalue_only=FALSE, n_perm=0,
                         B=10^3, B_MC=10^3, seed=123, use_b_approx=FALSE) {

    m <- length(S_X); n <- length(S_Y); N <- m + n
    ...
    if (!pvalue_only) {
        ...
        for (i in n:1) {
            if (is.character(g.hat)) {
                if (g.hat == "analytical") {
                    a_i <- sapply(1:(i + m), function(h)
                        (k + 1) * k_mom_beta(a = h, b = m + i - h + 1, k = k))
                } else {
                    cat("Error: g.hat must be either a density function or the string analytical.")
                }
            } else if (use_b_approx) {
                a_i <- g.hat((1:(m + i)) / (m + i + 1))
            } else {
            ...
            if (i == n) {
                pval.global <- compute.global.pvalue(
                    T.obs = stat_i, m = m, n = n, local_test = "g",
                    stats_G_vector = a_i, n_perm = n_perm, B = B, seed = seed
                )
            }
            if (d < n - i + 1) break
        }
        ...
        if (!is.null(S)) {
            if (decr) {
                ...
                X <- -S_X; Y_S <- -S_Y.S; Y_notS <- -S_Y.notS
            } else {
            ...
            if (use_b_approx) {
                a_N <- lapply((m + 1):(m + n), function(N_) g.hat((1:N_) / (N_ + 1)))
            } else {
            ...
            for (i in 1:s) {
                for (j in max(0, (h - s + i - 1)):0) {
                    ...
                    if (decr) RR <- base::rank(-ZZ) else RR <- base::rank(ZZ)
                    k <- length(ZZ) - m
                    stat_k <- sum(a_N[[k]][RR[(m + 1):(m + k)]])
                    mu_k   <- k * mean(a_N[[k]])
                    var_k  <- k * m * sum((a_N[[k]] - mean(a_N[[k]]))^2) /
                              ((m + k) * ((m + k) - 1))
        ...
        ## pvalue_only branch (only when S=NULL)
        if (is.character(g.hat)) {
            if (g.hat == "analytical") {
                a_i <- sapply(1:(n + m), function(h)
                    (k + 1) * k_mom_beta(a = h, b = m + n - h + 1, k = k))
            } else {
                cat("Error: g.hat must be either a density function or the string analytical.")
            }
        } else if (use_b_approx) {
            a_i <- g.hat((1:(m + n)) / (m + n + 1))
        } else {
...
#' d_G_cons
#'
#' Conservative (non-monotone) shortcut Shiraishi closed test.
#'
#' @param use_b_approx logical. See d_selection_G.
#'
#' @export
d_G_cons <- function(S_X, S_Y, S=NULL, g.hat, k=NULL, alpha=0.1,
                     pvalue_only=FALSE, n_perm=10, B=10^3, B_MC=10^3,
                     seed=123, use_b_approx=FALSE) {
    ...
    if (!pvalue_only) {
        ...
        while (cont == TRUE & l > 0) {
            if (is.character(g.hat)) {
                if (g.hat == "analytical") {
                    stats_G <- sapply(1:(l + m), function(h)
                        (k + 1) * k_mom_beta(a = h, b = m + l - h + 1, k = k))
                } else {
                    cat("Error: g.hat must be either a density function or the string analytical.")
                }
            } else if (use_b_approx) {
                stats_G <- g.hat((1:(m + l)) / (m + l + 1))
            } else {
                stats_G <- apply(replicate(B_MC,
                    sapply(X = sort(stats::runif(m + l)), FUN = g.hat)), 1, mean)
            }
            ...
            T_wc <- sum(R)
            crit <- asymptotic.critical.G(m = m, n = l,
                                          stats_G_vector = stats_G,
                                          alpha = alpha)

            if (l == s) {
                T.global <- T_wc
                pval.global <- compute.global.pvalue(
                    T.obs = T.global, m = m, n = s, local_test = "g",
                    stats_G_vector = stats_G,
        ...
        T.global <- sum(R)
        pval.global <- compute.global.pvalue(
            T.obs = T.global, m = m, n = n, local_test = "g",
            stats_G_vector = stats_G,
            n_perm = n_perm, B = B, seed = seed
        )
        d <- 0
