...
## -------------------------------------------------------------------------------------------------
## submit_exp_B1.sh — Grid launcher for the R experiment driver exp_B1_global_testing.sh.
##
## This script reproduces the global-testing simulation study reported in Appendix B1.
...
##   figA11      Figure A11
##
## Usage: ./submit_exp_B1.sh RESULT_KEY [--cluster|--local|--dry-run] [--force]
##   RESULT_KEY: one of the keys listed above (e.g. figA11)
    ...
    for N_TEST in "${N_TEST_LIST[@]}"; do
      for ALT in "${ALT_LIST[@]}"; do
        for PROP_OUT in "${PROP_OUT_LIST[@]}"; do
          ...
          # Script to be run
          SCRIPT="exp_B1_global_testing.sh $FIGURE $N_CAL $N_TEST $SEED $ALT $PROP_OUT"
