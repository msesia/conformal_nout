...
load_data <- function(setup) {
    ...
    if(names_ACODE) {
        method.labels <<- c("ACODE (Simes)", "ACODE (Storey-Simes)", "ACODE (Fisher)", "ACODE (WMW)", "ACODE (LMPR, Lehmann k=3)", "ACODE (LMPR,G-hat)", "ACODE (Adaptive)")
    } else {
        method.labels <<- c("Simes", "Storey-Simes", "Fisher", "WMW", "LMPR (Lehmann, k=3)", "LMPR (G-hat)", "Adaptive") #"WMW (k=3)",
    }
...
plot_lb_1 <- function() {
    ...
    method.labels <- c("Fisher", "WMW", "LMPR (oracle)", "LMPR (G-hat, monotone)")
    alternative.values <- c("uniform", "lehmann_k2", "beta_0.25_0.25", "beta_10_10", "normal_1.5_1", "normal_-1.5_1", "normal_0_2", "normal_0_0.25")
    alternative.labels <- c("Uniform (null)", "Lehmann (k=3)", "Beta (overdispersed)", "Beta (underdispersed)", "Normal (positive shift)", "Normal (negative shift)",
                            "Normal (overdispersed)", "Normal (underdispersed)")
    ## Manual color and shape scales
    cbPalette <<- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#6e57d2", "red")
    colors <- c("Fisher" = cbPalette[4],
                "WMW" = cbPalette[3],
                "LMPR (oracle)" = cbPalette[6],
                "LMPR (G-hat, monotone)" = cbPalette[7]) # Darker green
    shapes <- c("Fisher" = 3, "WMW" = 1, "LMPR (oracle)" = 8,
                "LMPR (empirical)" = 15, "LMPR (G-hat, monotone)" = 16)
...
#            geom_hline(yintercept=15, alpha=0, linetype = 2) +
        ...
        filename <- sprintf("figures/lb_ncal%d_ntest%d.pdf", n_cal.plot, n_test.plot)
...
plot_power_1 <- function(setup) {
    ...
    method.labels <- c("Fisher", "WMW", "LMPR (oracle)", "LMPR (G-hat)", "LMPR (G-hat, monotone)")
    ...
    colors <- c("Fisher" = cbPalette[4],
                ...
                "LMPR (G-hat, monotone)" = cbPalette[7]) # Darker green
    shapes <- c("Fisher" = 3, "WMW" = 1, "LMPR (oracle)" = 8,
                "LMPR (G-hat)" = 15, "LMPR (G-hat)" = 16, "LMPR (G-hat, monotone)" = 16)
...
##            geom_errorbar(aes(ymin = Power - 2*SE, ymax = Power + 2*SE), width = 0.02) +
            ...
            theme(legend.position = "bottom")
        ## Save the plot as a PNG file
        filename <- sprintf("figures/power_ncal%d_ntest%d.pdf", n_cal.plot, n_test.plot)
        ggsave(filename = filename, plot = pp, width = 10, height = 5)
    }
