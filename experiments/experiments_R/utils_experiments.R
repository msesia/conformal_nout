...
run_global_testing <- function(data, alternative=NULL) {
    ...
    pval.wmw <- d_selection_higher(S_X = S_X, S_Y = S_Y, local_test = "WMW", n_perm = 0, pvalue_only = TRUE)$global.pvalue

    ## Compute global p-value with WMW test (k=2)
    pval.wmw.2 <- d_selection_higher(S_X = S_X, S_Y = S_Y, local_test = "higher", k = 2, n_perm = 0, pvalue_only = TRUE)$global.pvalue

    ## Compute global p-value with WMW test (k=3)
    pval.wmw.3 <- d_selection_higher(S_X = S_X, S_Y = S_Y, local_test = "higher", k = 3, n_perm = 0, pvalue_only = TRUE)$global.pvalue
    ...
    pval.g.hat.1 <- compute.global.pvalue.shirashi.adaptive(S_X, S_Y, prop_cal=0.5, num_mc=1000, fit_method="betamix")

    ## Apply Shirashi's method using g-hat estimated through beta mixture (increasing)
    pval.g.hat.2 <- compute.global.pvalue.shirashi.adaptive(S_X, S_Y, prop_cal=0.5, num_mc=1000, fit_method="betamix", monotone=TRUE)

    ## ## Apply Shirashi's method using g-hat estimated through mixmodel
    ## pval.g.hat.3 <- compute.global.pvalue.shirashi.adaptive(S_X, S_Y, prop_cal=0.5, num_mc=1000, fit_method="mixmodel")

    ## ## Apply Shirashi's method using g-hat estimated through mixmodel (increasing)
    ## pval.g.hat.4 <- compute.global.pvalue.shirashi.adaptive(S_X, S_Y, prop_cal=0.5, num_mc=1000, fit_method="mixmodel", monotone=TRUE)
...
run_outlier_enumeration <- function(data, alpha=0.1, alternative=NULL) {
    ...
    ## Compute global p-value with WMW test
    res.wmw <- d_selection_higher(S_X = S_X, S_Y = S_Y, alpha=alpha, local_test = "WMW", n_perm = 0)

    ## Compute global p-value with WMW test (higher order)
    res.wmw.2 <- d_selection_higher(S_X = S_X, S_Y = S_Y, alpha=alpha, local_test = "higher", k=2, n_perm = 0)

    ## Compute global p-value with WMW test (higher order)
    res.wmw.3 <- d_selection_higher(S_X = S_X, S_Y = S_Y, alpha=alpha, local_test = "higher", k=3, n_perm = 0)
