## -----------------------------------------------------------------------------
## Required results : fig3
## Produces         : Figure 3, Figure A3
##
## Required results : figA6
...
make_plot_lower_bound_proportion_sel <- function(fig.name, plot.quantile=0.5) {
    ...
    plot.file.1 <- sprintf("figures/%s_lower_bound_sel_q%s.pdf", fig.name, plot.quantile)
    ggsave(pp, file=plot.file.1, height=2.25, width=7, units="in")
}

## ---- Run --------------------------------------------------------------------

## Setup 1001: circles-mixed synthetic data, selective enumeration
results <- load_data("fig3")

## Paper Figure 3: median lower bound for selected subsets
make_plot_lower_bound_proportion_sel(fig.name="fig3", plot.quantile=0.5)

## Appendix Figure A3: 90th-quantile version
make_plot_lower_bound_proportion_sel(fig.name="figA3", plot.quantile=0.9)


## Figure A6: binomial synthetic data, selective enumeration (appendix)
results <- load_data("figA6")

make_plot_lower_bound_proportion_sel(fig.name="figA6", plot.quantile=0.5)
make_plot_lower_bound_proportion_sel(fig.name="figA8", plot.quantile=0.9)
