...
##     pi.not = mixmodel::mix.model(scores_pooled, method = "fixed",
##                                  c.n = .05*log(log(length(scores_pooled))), gridsize = 600)$alp.hat
...
asymptotic.pvalue.G <- function(m, n, stats_G_vector, T.obs) {
  ...
  if(variance.TG>0) {
      print(c(T.obs, mean.TG, sqrt(variance.TG)))
