...
> **Platform.** This code was developed and tested on Linux. macOS should work for the local (non-cluster) workflow, though shell scripts assume GNU coreutils and bash 4+ (install via `brew install bash coreutils`); the cluster workflow requires Slurm. Windows is not supported.
